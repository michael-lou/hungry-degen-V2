...
async function main() {
  ...
  let blackGhostNFTAddress: string;

  try {
    // 第一步：检查并部署 BlackGhostNFT
    console.log('\n🔍 步骤1: 检查 BlackGhostNFT 部署状态...');
    
    const existingBlackGhostNFT = addressManager.getContractAddress(networkName, 'BlackGhostNFT');
    
    if (existingBlackGhostNFT) {
      console.log(`✅ BlackGhostNFT 已存在: ${existingBlackGhostNFT.address}`);
      blackGhostNFTAddress = existingBlackGhostNFT.address;
    } else {
      console.log('\n🚀 部署 BlackGhostNFT...');
      ...
      blackGhostNFTAddress = await blackGhostNFT.getAddress();
    ...
    // 第二步：部署 BlackGhostSale
    console.log('\n🚀 步骤2: 部署 BlackGhostSale...');
    ...
    // 后续配置建议
    console.log('\n📝 后续配置建议:');
    console.log('1. 运行 setup-BlackGhostSale-referralV2.ts 配置推荐系统');
    console.log('2. 设置 DUST Token: blackGhostSale.setDustToken(dustTokenAddress)');
    console.log('3. 设置 Character NFT: blackGhostSale.setCharacterNFT(characterNFTAddress)');
    console.log('4. 配置白名单: blackGhostSale.setWhitelist([addresses], [discounts])');
    console.log('5. 根据需要调整销售参数');
