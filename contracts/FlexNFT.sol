...
contract FlexNFT is Initializable, ERC721EnumerableUpgradeable, ERC2981Upgradeable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    // NFT属性存储
    mapping(uint256 => Flex) private flexItems;
    ...
    // 事件
    event FlexMinted(address indexed to, uint256 indexed tokenId, uint8 characterType, uint8 rarity);
    ...
    modifier onlyAuthorized() {
        require(owner() == _msgSender() || authorizedCallers[_msgSender()], "FlexNFT: Not authorized");
        _;
    }

    function initialize() public initializer {
        __ERC721_init("HungryDegenFlex", "HDFI");
    ...
    function mint(
        ...
        emit FlexMinted(to, tokenId, characterType, rarity);
    ...
    function getFlexInfo(uint256 tokenId) external view returns (Flex memory) {
        ...
        return flexItems[tokenId];
    }

    function getFlexInfoBatch(uint256[] memory tokenIds) external view returns (Flex[] memory) {
        Flex[] memory items = new Flex[](tokenIds.length);
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(_exists(tokenIds[i]), "Token does not exist");
            items[i] = flexItems[tokenIds[i]];
        }
        return items;
    }
    ...
    function updateLevel(uint256 tokenId, uint8 newLevel) external onlyAuthorized {
        require(_exists(tokenId), "Token does not exist");
        flexItems[tokenId].level = newLevel;
    ...
    function updateWear(uint256 tokenId, uint8 newWear) external onlyAuthorized {
        require(_exists(tokenId), "Token does not exist");
        flexItems[tokenId].wear = newWear;
